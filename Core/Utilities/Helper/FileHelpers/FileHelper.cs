...
namespace Core.Utilities.Helper.FileHelpers
{
    public class FileHelper
    {
        ...
        {
            var fileExists = CheckFileExists(file); // file varlık kontrolü yapılır.
            if (fileExists.Message != null)
            {
                return new ErrorResult(fileExists.Message);
            }

            var type = Path.GetExtension(file.FileName); // resmin tipini alır .jpg, .jpeg, .img gibi.
            var typeValid = CheckFileTypeValid(type); //  dosyanın tipi kontrol edilir.
            var randomName = Guid.NewGuid().ToString(); // random bir guid ile isim alınır.
            ...
            CheckDirectoryExists(_currentDirectory + _folderName); // wwwroot / images altında dosya varlık kontrolü yapılır.
            CreateImageFile(_currentDirectory + _folderName + randomName + type, file); //resim kopyalanır.
